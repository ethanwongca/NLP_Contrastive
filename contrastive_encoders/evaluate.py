...
class InformationRetrievalEvaluator(SentenceEvaluator):
    ...
    def __call__(
        ...
        epoch: int = -1, 
        steps: int = -1, 
        query_embeddings: Tensor | None = None,  # MOD
        corpus_embeddings: Tensor | None = None,  # MOD
        *args, 
        **kwargs
    ) -> dict[str, float]:
        
        self.ext_query_embeddings = query_embeddings # MOD: store the external query_embeddings if provided
        self.ext_corpus_embeddings = corpus_embeddings # MOD: store the external corpus_embeddings if provided
    ...
    def compute_metrices(
        ...
        # MOD: if embeddings are given, use them directly
        if getattr(self, "ext_query_embeddings", None) is not None:
            query_embeddings = self.ext_query_embeddings
            corpus_embeddings = self.ext_corpus_embeddings
            
            assert len(query_embeddings) == len(self.queries_ids)
            assert len(corpus_embeddings) == len(self.corpus_ids)
